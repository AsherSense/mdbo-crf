...
test("chronological navigation covers every stored module without isolated QoL, medication or supplemental pages",()=>{
 assert.deepEqual(WORKFLOW.filter(v=>!v.event).map(v=>v.id),["screen","baseline","surgery","early","month3","month6","month12"]);
 ...
 assert.deepEqual(WORKFLOW.find(v=>v.id==="early").panes.slice(0,5).map(p=>p.context||p.module),["7天","week2","21天","28天","month1"]);
